...
const componentResolver = (type: string) => {
  switch (type) {
    case 'date':
      return resolveComponent('TableFiltersFilterDate')
    case 'autocomplete':
      return resolveComponent('TableFiltersFilterAutocomplete')
    case 'search':
      return resolveComponent('TableFiltersFilterSearch')
    case 'select':
      return resolveComponent('TableFiltersFilterSelect')
    case 'number':
      return resolveComponent('TableFiltersFilterNumber')
    case 'text':
    default:
      return resolveComponent('TableFiltersFilterText')
  }
}
      ...
      <div class="flex items-center gap-2">
        <template v-for="action in validActions(actionToolbars, {})" :key="action.key">
          <button
            ...
            class="w-8 h-8 flex items-center justify-center rounded-lg bg-white/20 hover:bg-white/30 text-white transition-colors disabled:opacity-50"
            :disabled="actionLoading[action.key]"
            @click="$emit(action.emit as any)"
            :title="action.tooltip"
          >
            <svg v-if="actionLoading[action.key]" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
          ...
          >
            ...
            >
              <template v-if="f.type === 'search' && addAction" #append>
                <button
                  class="absolute right-12 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors"
              ...
              <td v-if="actions.length > 0" class="px-4 py-3">
                <div class="flex items-center justify-center gap-2">
                  <template v-for="action in validActions(actions, item)" :key="action.key">
                    <button
                      :style="{ backgroundColor: getActionColor(action.color) }"
                      class="w-7 h-7 flex items-center justify-center text-white rounded transition-colors hover:opacity-80"
                      @click.stop="$emit(action.emit as any, item)"
                      :title="action.tooltip"
                    >
                      <i :class="['mdi', action.icon, 'text-base']" />
                    </button>
                  </template>
                </div>
              </td>
