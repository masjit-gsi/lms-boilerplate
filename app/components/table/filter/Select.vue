  ...
  <div ref="wrapperRef" class="space-y-1 relative">
    ...
    <div class="relative">
      ...
      >
        <div
          v-if="isOpen"
          class="absolute z-50 w-full mt-1 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 max-h-60 overflow-hidden"
        >
          <ul class="overflow-y-auto py-1">
            <li v-if="items.length === 0" class="px-4 py-3 text-sm text-slate-500 text-center">
              Tidak ada opsi
            </li>
            <li v-for="option in items" :key="option[valueKey]">
              <button
                type="button"
